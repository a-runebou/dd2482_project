...
export function CalendarSourcesPage() {
  ...
  const refresh = useMutation(
    refreshCalendarSourceMutationOptions(queryClient),
  );
  ...
  return (
    <PageContainer>
      <div className="flex flex-wrap items-start justify-between gap-2">
        <PageHeading title="Calendar sources" />
        <Link className={`mt-1 ${LINK}`} to="/groups">
          Back to groups
        </Link>
      </div>
      ...
      <Card className="mt-6">
        ...
        >
          <label className="block text-sm font-medium" htmlFor="calendar-url">
            Calendar URL
          </label>
          <input
            id="calendar-url"
            className="w-full rounded-md border border-neutral-400 px-3 py-2"
            type="url"
            required
            value={url}
            onChange={(event) => setUrl(event.target.value)}
          />
          <label className="block text-sm font-medium" htmlFor="calendar-label">
            Label{" "}
          ...
          <Button type="submit" variant="primary" disabled={create.isPending}>
            {create.isPending && <Spinner />}Add URL calendar
          </Button>
        </form>
        {create.error !== null && (
          <SourceError
      ...
      <Card className="mt-6">
        ...
        <p className="mt-2 text-sm text-neutral-600">
          The server parses the file and imports its events.
        </p>
        <label
      ...
      <section className="mt-8" aria-labelledby="existing-calendars-heading">
        <h2 id="existing-calendars-heading" className="text-xl font-semibold">
          Your calendars
        </h2>
        {query.data.data.length === 0 ? (
          <p className="mt-3 text-neutral-600">No calendars imported yet.</p>
        ) : (
            ...
            {query.data.data.map((source) => (
              <Card key={source.id}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <SourceStatus
                      ...
                      errorCode={source.last_error_code}
                    />
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    >
                      {refresh.isPending && <Spinner />}Refresh
                    </Button>
                    <Button
                ...
                {refresh.error !== null && (
                  <SourceError
                    error={refresh.error}
                    retry={() => refresh.mutate(source.id)}
                    isRetrying={refresh.isPending}
                  />
                )}
                {remove.error !== null && (
                  <SourceError
                    error={remove.error}
                    retry={() => remove.mutate(source.id)}
                    isRetrying={remove.isPending}
                  />
                )}
              </Card>
            ))}
